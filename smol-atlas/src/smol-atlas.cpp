...
struct smol_pool_t
{
    ...
    smol_pool_t(size_t size_in_items)
        : m_chunk_size_in_items(size_in_items), m_cur_chunk(new chunk_t(size_in_items)), m_free_list(m_cur_chunk->storage)
    {
    }
    ~smol_pool_t()
    {
...
void sma_atlas_clear(smol_atlas_t *atlas, int new_width, int new_height)
{
    atlas->clear();
    if (new_width > 0)
        atlas->m_width = new_width;
    if (new_height > 0)
        atlas->m_height = new_height;
}
