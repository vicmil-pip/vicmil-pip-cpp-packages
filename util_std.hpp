...
#define Print(x)                                                                                                                                                  \
    {                                                                                                                                                             \
        const std::string line_info = vicmil::pad_str(vicmil::split_string(__FILE__, '/').back(), 20) + ":" + vicmil::pad_str(std::to_string(__LINE__), 4) + ":"; \
...
#define AddTest(test_name) \
    TestWrapper(test_name##_, void test() { test_name(); });
...
*/
    ...
    */
    ...
    public:
        ...
        FileManager(std::string filename_, bool create_file = false)
        { // class constructor
            filename = filename_;
            if (create_file)
            {
        ...
        std::vector<char> read_bytes(unsigned int read_size_in_bytes)
        { // read bytes as binary and write into &output
        ...
        std::vector<unsigned char> read_bytes_uchar(unsigned int read_size_in_bytes)
        { // read bytes as binary and write into &output
